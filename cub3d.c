...
int check_xpm(t_data *data)
{
	...
	if (data->no[len - 1] != 'm' || data->no[len - 2] != 'p' || data->no[len - 3] != 'x' || data->no[len - 4] != '.')
	{
		ft_putstr_fd("Error\nInvalid file extension1\n", 2);
		return (1);
	}
	len = ft_strlen(data->so);
	if (data->so[len - 1] != 'm' || data->so[len - 2] != 'p' || data->so[len - 3] != 'x' || data->so[len - 4] != '.')
	{
		ft_putstr_fd("Error\nInvalid file extension2\n", 2);
		return (1);
	}
	len = ft_strlen(data->we);
	if (data->we[len - 1] != 'm' || data->we[len - 2] != 'p' || data->we[len - 3] != 'x' || data->we[len - 4] != '.')
	{
		ft_putstr_fd("Error\nInvalid file extension3\n", 2);
		return (1);
	}
	len = ft_strlen(data->ea);
	if (data->ea[len - 1] != 'm' || data->ea[len - 2] != 'p' || data->ea[len - 3] != 'x' || data->ea[len - 4] != '.')
	{
...
{
	if (ft_strncmp(data->map[*i], "NO", 2) == 0 || ft_strncmp(data->map[*i], "SO", 2) == 0 || ft_strncmp(data->map[*i], "WE", 2) == 0 || ft_strncmp(data->map[*i], "EA", 2) == 0 || ft_strncmp(data->map[*i], "F", 1) == 0 || ft_strncmp(data->map[*i], "C", 1) == 0)
	{
		return (1);
	}
	return (0);
